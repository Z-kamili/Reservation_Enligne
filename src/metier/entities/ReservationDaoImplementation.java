...
public class ReservationDaoImplementation implements IReservationDao {
	...
	public List<Reservation> getAllReservationAccpet(int id) throws SQLException {
		// TODO Auto-generated method stub
				...
				Connection connection = DAO.entities.Connection.getConnection();
				PreparedStatement ps = connection.prepareStatement("SELECT id, email, \"Desc\", \"Date\", active, \"Id_user\"\r\n" + 
						"FROM public.\"Reservation\" where \"Id_user\" = ? and active = ?;");
				ps.setInt(1, id);
	...
	public List<Reservation> PriductParMC(String mc) throws SQLException {
		...
		PreparedStatement ps = connection
				.prepareStatement("SELECT id, title, image, prix FROM public.produit where title Like ?");
		ps.setString(1, "%" + mc + "%");
		ResultSet rs = ps.executeQuery();
	...
	public List<Reservation> update(int id) throws SQLException {
		...
		Connection connection = DAO.entities.Connection.getConnection();
		PreparedStatement ps = connection
				.prepareStatement("UPDATE public.\"Reservation\"\r\n" + "	SET active=?" + "WHERE id = ?");
	...
	public Reservation getProduct(int id) {
		...
		try {
			PreparedStatement ps = connection
					.prepareStatement("SELECT id, title, image, prix FROM public.produit where id = ?");
			ps.setInt(1, id);
			ResultSet rs = ps.executeQuery();
	...
	public List<Reservation> getAllReservation() {
		...
		try {
			PreparedStatement ps = connection
					.prepareStatement("SELECT id, email, \"Desc\", \"Date\"\r\n" + "FROM public.\"Reservation\";");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				Reservation a = new Reservation();
				a.setId(rs.getInt("id"));
	...
	public List<Reservation> getAllReservation_false() {
		...
		List<Reservation> rsv = new ArrayList<Reservation>();
		try {
			PreparedStatement ps_2 = connection.prepareStatement(
					"SELECT id, email, \"Desc\", \"Date\"\r\n" + "FROM public.\"Reservation\" where active = false;");
